...
function renderGeoCardSystem(data){
 ...
 ['📏','Distancia mínima',isFinite(minDist)?fmtKm(minDist):'—','Al grupo más cercano','#22c55e']
 ];
 ...
 const sitesRows = links.slice(0,5).map((l,idx)=>{const d=extractGroupData(l);const st=getDictamen(l.link_type,(l.distance_border_m||l.distance_m||0)/1000);const areaM2=computeFeatureAreaM2(l.feature)||0;const areaKm2=areaM2/1e6;const perimeterKm=computePerimeterKm(l.feature);const deqKm=calcDiametroEquivalenteKm(areaKm2);return `<tr><td>${idx+1}</td><td>${d.nombre}</td><td>${d.sistema||'SNASPE'}</td><td>${fmtKm(l.distance_m)}</td><td>${fmtArea(areaM2)}</td><td>${formatKm(perimeterKm)}</td><td>${formatKm(deqKm)}</td><td><span class='badge badge--${st.class}'>${st.text}</span></td></tr>`}).join('');
 const techRows = links.map((l,idx)=>{const d=extractGroupData(l);const st=getDictamen(l.link_type,(l.distance_border_m||l.distance_m||0)/1000);const areaM2=computeFeatureAreaM2(l.feature)||0;const areaKm2=areaM2/1e6;const perimeterKm=computePerimeterKm(l.feature);const deqKm=calcDiametroEquivalenteKm(areaKm2);return `<tr><td>${idx+1}</td><td>${d.nombre}</td><td>${d.sistema||'SNASPE'}</td><td>${fmtKm(l.distance_m)}</td><td>${fmtArea(areaM2)}</td><td>${formatKm(perimeterKm)}</td><td>${formatKm(deqKm)}</td><td><span class='badge badge--${st.class}'>${st.text}</span></td></tr>`}).join('');
 const headCols=`<tr><th>ID</th><th>Sitio protegido</th><th>Categoría</th><th>Distancia al perímetro</th><th>Superficie</th><th>Perímetro</th><th>Diámetro equivalente</th><th>Estado</th></tr>`;
 const viewAll = links.length>5?`<div class='muted'>Ver todos (${links.length})</div>`:'';
 const t=document.getElementById('sitesTable'); if(t) t.innerHTML=`<table><thead>${headCols}</thead><tbody>${sitesRows}</tbody></table>${viewAll}`;
 const tech=document.getElementById('techTableWrap'); if(tech) tech.innerHTML=`<table><thead>${headCols}</thead><tbody>${techRows}</tbody></table>`;
